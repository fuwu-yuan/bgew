...
export class Inputtext extends Entity {
    protected _inputDispatcher = new Dispatcher();
    // Padding
    private _padding: { top: number; right: number; bottom: number; left: number } = { top: 2, left: 2, right: 2, bottom: 2 }
    // Normal
    ...
    onchange(cb: (evt: FocusEvent) => void) {
        this._inputDispatcher.on("change", cb);
    }
    onfocus(cb: (evt: FocusEvent) => void) {
        this._inputDispatcher.on("focus", cb);
    }
    onblur(cb: (evt: FocusEvent) => void) {
 ...
 */
export class ExperimentalInputtext extends Inputtext {}
